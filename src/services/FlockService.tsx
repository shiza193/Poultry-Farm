...
export const addFlock = async (payload: AddFlockPayload): Promise<AddFlockResponse> => {
  try {
...
export const deleteFlock = async (flockId: string): Promise<{ status: string; message: string }> => {
  try {
...
export const getFlockHealthRecords = async (
  ...
  try {
    ...
    const response = await api.post('api/Flock/get-flock-health-records', payload);

    if (response.data.status === 'Success') {
      return response.data;
    } else {
      throw new Error(response.data.message || 'Failed to fetch flock health records');
    }
  } catch (error: any) {
    console.error('Error fetching flock health records:', error.response || error.message);
    throw error;
  }
};
...
export const addAutomaticFeedRecord = async (
  ...
  try {
    ...
    if (responseData?.status === 'Success') {
      ...
      throw new Error(responseData?.message || 'Failed to add feed record consumption');
    }
  } catch (error: any) {
    console.error('Add Automatic Feed Record API Error:', error.response || error.message);
    throw error;
  }
};
...
export const addFlockHealthRecord = async (
  ...
  try {
    ...
    if (response.data.status === 'Success') {
      ...
      throw new Error(response.data.message || 'Failed to add flock health record');
    }
  } catch (error: any) {
    console.error('Add Flock Health Record API Error:', error.response || error.message);
    throw error;
  }
};
...
export const addHospitality = async (
  ...
  try {
    ...
    if (response.data.status === 'Success') {
      ...
      throw new Error(response.data.message || 'Failed to add hospitality record');
    }
  } catch (error: any) {
    console.error('Add Hospitality API Error:', error.response || error.message);
    throw error;
  }
};
...
export const updateFlockIsEnded = async (
  ...
  try {
    ...
    console.error('Update Flock isEnded API Error:', error.response || error.message);
    throw error;
  }
};
...
export const getFlockStock = async (businessUnitId: string): Promise<FlockStock[]> => {
  try {
    if (!businessUnitId) throw new Error("Business Unit ID is required");

    const response = await api.get<FlockStockResponse>(`api/Flock/get-flock-stock/${businessUnitId}`);
    ...
    console.error("Error fetching flock stock:", error.response || error.message);
    throw error;
  }
};
...
export const getSalesByFilter = async (filters: GetSaleFilters): Promise<SaleRecord[]> => {
  try {
    // Only send keys that have value
    const payload: any = {
      pageNumber: filters.pageNumber || 1,
...
export const getHospitalityByFilter = async (
  ...
  try {
    ...
    if (response.data.status === 'Success') {
      ...
      throw new Error(response.data.message || 'Failed to fetch hospitality data');
    }
  } catch (error: any) {
    console.error('Error fetching hospitality data:', error.response || error.message);
    throw error;
  }
};
...
export const addSale = async (payload: AddSalePayload): Promise<AddSaleResponse> => {
  try {
    const response = await api.post<AddSaleResponse>('api/Sale/add-sale', payload);
